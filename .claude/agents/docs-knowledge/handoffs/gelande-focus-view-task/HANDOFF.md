...
**from**: `product-ux`  
**保存日**: 2026-03-28  
**最終更新**: 2026-06-22（サイド RM **フェーズ 6 クローズ** — S-4 Plan B + 0-D + snow-data v1）  
...
**カレント HTML は mock-016**（015 評価 P0 是正版。011 系思想・013 i18n 制度を継承）。015 は `mock-archive/mock-015.html`、014/013 も `mock-archive/` 参照。
...
| 種別 | パス |
|------|------|
| **カレントモック** | `.claude/agents/product-ux/mock-016.html` |
| mock-015（アーカイブ・016 の直前版） | `.claude/agents/product-ux/mock-archive/mock-015.html` |
| mock-014（アーカイブ） | `.claude/agents/product-ux/mock-archive/mock-014.html` |
...
|------------|------|----------------|--------------|
| `FocusView_FromRanking` | トップランキング等・一覧行タップ | **mock-016**（011 系思想） | **016 型テンプレ専用**。012 型 DOM と混在させない。 |
...
**リスク（混在禁止の理由）**: 現状モバイル地図詳細は `selectResort` が **単一の `contentHtml`** に日付帯・時間帯バー等を載せており、見た目が 012 に近くなりやすい。ランキング経路で mock-016 を載せる場合、**同一 DOM に両テンプレを足し込まない**こと。
...
| 優先度 | 項目 | 想定担当 | 016 での状態 |
|--------|------|----------|--------------|
| **P0** | 上部余白（map-strip と sheet の間/前に不要空白が出る） | implementer | モック上は解消。本番未検証 |
| **P0** | はみ出し・切れ（Overflow） | implementer | モック上は 11px 最小・EN scroll-snap 対応。本番 320/390 未検証 |
| **P0** | ゴー/リスクの算法と文言 | snow-data / 実装 | **v1 完了** — `GLANCE_THRESHOLDS` + FromMap Plan B（S-4） |
| **P0** | QA 再評価ゲート（mock-016 Pass/Fail） | qa-accessibility | **Pass（第7回 2026-06-22）** |
| **P1** | B/A の永続・自動判定の要否 | product-ux / 実装 | 未解決 |
| **P1** | ドキュメント正本の一本化（screen-flow / MVC-06） | docs-knowledge | **完了（2026-06-22）** — 正本 mock-016、MVC-06 3 段 |
| **P1** | FromMap マップクローム（S-2〜S-4） | implementer | **完了（2026-06-22）** — [ROADMAP-frommap-chrome-side.md](./ROADMAP-frommap-chrome-side.md) |
| **P1** | デスクトップ popup 簡素化（0-D） | implementer | **完了（2026-06-22）** |
| **P2** | 薄マップ帯からの「マップ全画面」導線の有無 | product-ux | 未解決 |

### mock-016 で解消した評価 P0（モック上）

- B glance 判定 UI（状態語 18px + 4px 信号色 + cm 第2行）
- B デフォルト: `#l2-block` 初期 `hidden` + load 時 `setMode(true)`
- day-chip: `button[role=tab]` + キーボード + `aria-selected` 全5件
- 最小フォント 11px（アプリ UI 内）
- EN 375px: `day-chips` scroll-snap mandatory + 右端 `···` affordance
- 013 水準のミニマル化（テクスチャ / shadow / glow 削減）
- ランキング→フォーカス 2 フレーム流入図（注釈列）
- **← トップ**（`.btn-top-sm`、ranking 経路のみ）
- **FromMap 2 フレーム**（S-1: マップ閲覧 / ピン詳細注釈）
...
- **EN**: `mock-016.html` の `lang="en"`（013 制度：`--slot-w`、2 行ラベル、雨バッジ折返し、`labelShort` / `labelAria`、375 scroll-snap）を満たす。
- **A11y**: `mock-016.html` のタッチ目安（44px 系）・`--muted-caption`・11px 最小・tab キーボード・`prefers-reduced-motion` を本番で再現する。
- **日付チップ（MVC-06）**: 各 `.day-chip` は `button[role=tab]` 可で「相対ラベル `span` + `.day-chip__cal` + `strong`（`N cm` / `—`）」の **3 段**。暦日欠落は **MVC-06 違反**（旧 MVC-17 は map-view-correction HANDOFF で統合済み）。**欠損時のみ** `—`、0 は `0 cm`。
- **日付チップ数**: FocusView_FromRanking は **今日〜+4の5チップ**（mock-016 準拠）。
- **B glance 判定**: 状態語 + 信号色（GO/注意/厳しい）。算法は snow-data と合意後に接続（016 はダミー）。
- **温度チャート非表示**: FocusView_FromRanking は `tl-legend` / `tl-scroll` までで完結（mock-016 準拠）。
...
| **implementer** | **フェーズ 4 QA 合格後** — `selectResort` 分岐・**mock-016** DOM 移植（ゲート遵守） |
| **qa-accessibility** | mock-016 再評価（375/390/700）— フェーズ 4 |
...
## mock-015（アーカイブ・2026-03-28）

**パス**: `mock-archive/mock-015.html`。014 継承 + frontend-design 深度。016 で評価 P0 是正のためアーカイブ。

---

## mock-016（カレント・2026-06-22）

**前提**: mock-015 をアーカイブ。**是正ロードマップ フェーズ 2**（評価 P0 反映）。

| 項目 | 内容 |
|------|------|
| B glance | 18px 状態語 + 4px 信号色（`--signal-go/caution/nogo`）+ cm 第2行。❄️ 廃止 |
| B 初期 | `#l2-block hidden` + `setMode(true)` on load |
| day-chip | `button[role=tab]`、`aria-selected` 全5件、roving tabindex + 矢印キー |
| タイポ | アプリ UI 最小 **11px**（8–9px 廃止） |
| EN 375 | `day-chips` scroll-snap mandatory + 右端 `···` フェード |
| ミニマル | 013 水準（map-strip テクスチャ・glance shadow・chip グロー・sheet 入場削除） |
| 流入 | 注釈列に **[ランキング] → [フォーカス]** 2 フレーム図 |
| 生活者語 | L2「前日予報」「強い風」（D-1/極ドライ風を置換） |
| ドキュメント | docs-knowledge フェーズ 1 完了 — `screen-flow.md` / map-view-correction **MVC-06 3 段**に正本統一 |

**次ゲート**: qa-accessibility S-3（FromMap chrome）→ snow-data フェーズ 3 合意 → S-4。

**サイド RM**: [ROADMAP-frommap-chrome-side.md](./ROADMAP-frommap-chrome-side.md)
...
3. **P2 L1/L2/L3** — mock-016 準拠のブロック移植（012 と共有しない）。  
4. **P3 i18n** — EN は mock-016（013 制度）。  
5. **P4 a11y** — mock-016 の 44px・11px 最小・tab キーボード・reduced-motion を本番 CSS に反映。  
...
```text
[HANDOFF]
from: product-ux + docs-knowledge
task: ゲレンデフォーカスビュー・タスク（gelande-focus-view-task）
決定事項:
  - ランキングからの一発フォーカスは mock-011 系を正とする（薄マップ帯・シート・L1/L2/L3・タイトル横の小「次 →」）。
  - 英語 UI は mock-013 由来の lang 別トークン・2 行ラベル・aria 完全形に従う（カレント HTML は mock-016）。
  - mock-016 は 015 評価 P0 是正版（B 判定 glance・tab 化 day-chip・11px 最小・013 ミニマル・2 フレーム流入図）。015 は mock-archive。
  - 日付チップは map-view-correction MVC-06: 3 段（相対 span + .day-chip__cal + strong）。旧 MVC-17 は統合済み。
  - screen-flow.md / map-view-correction HANDOFF の正本を mock-016 + MVC-06 に統一（docs-knowledge フェーズ 1 完了）。
  - 本番実装は mock-016 の QA 合格（フェーズ 4）まで着手しない。
...
モックファイル: mock-016.html（カレント）／mock-archive/mock-015.html／mock-archive/mock-014.html
本番当たり: 本ファイル「本番コードの当たり」＋ architecture/touch-points.md
未解決（優先度）:
  - P0: ゴー/リスクの算法と文言（snow-data フェーズ 3）。
  - P0: qa-accessibility による mock-016 再評価（フェーズ 4）。
  - P0: 本番上部余白・Overflow（implementer — モック合格後）。
  - P1: B/A の永続・自動判定の要否。
  - P2: 薄マップ帯からの「マップ全画面」導線の有無。
実装エージェントへの指示:
  - 本番は **フェーズ 4 QA 合格後**、implementer ゲートで依頼。DOM/スタイルの基準は mock-016。
  - 016 で解消済み（モック）: B glance 判定、L2 初期 hidden、day-chip tab 化、11px 最小、EN scroll-snap、013 ミニマル、2 フレーム流入図。
```
