...
**同一 DOM に「ランキングフォーカス用（011/016 系）」と「マップ大型シート案（012）」を混在させない。** 流入経路ごとにテンプレート／分岐を分ける（詳細: `handoffs/gelande-focus-view-task/HANDOFF.md`）。

```text
                    ┌─ [トップランキング・一覧] ─ 行タップ（一発）
                    │         │
                    │         ▼
                    │   FocusView_FromRanking  （正: mock-016）
...
- **正本モック**: `mock-016.html` の `.device-screen` 内
- **日付チップ形状（MVC-06）**: 相対ラベル `span` ＋ `.day-chip__cal` ＋ `strong`（降雪）の **3 段**。要素型は `button[role=tab]` 可（mock-016 準拠）
...
- **カレントモック**: `mock-016.html`（011 系思想 + 013 i18n 制度 + 評価 P0 是正）
- **公式フロー（主）**:
...
- **注**: ゲレンデフォーカス経路の正は上記 **ゲレンデフォーカスビュー・タスク** と `mock-016`。
- **S-2（2026-06-22）**: FromMap マップ閲覧はコンパクト chrome、ピン詳細は上段地図プレビュー＋012 シート。本番 `recenterMapForFromMapDetail`。詳細 [ROADMAP-frommap-chrome-side.md](../docs-knowledge/handoffs/gelande-focus-view-task/ROADMAP-frommap-chrome-side.md)。
