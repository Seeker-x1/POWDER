...
> **メモ:** ローカルの作業フォルダは、以前の `OneDrive\デスクトップ\CLOUDE\POWDER` から **`デスクトップ\Cloude\JAPOWSERCH`** に移しました。以下のパス例は **JAPOWSERCH** を前提にしています。サイトのブランド名は **Japowserch** で、GitHub のリポジトリ名は例として **`japowserch`** を使います（既に別名で公開済みなら、その名前のままで構いません）。

GitHub アカウントを作ったあと、このフォルダ（**JAPOWSERCH**）を GitHub に上げて誰でも見られるようにする方法です。
...
2. 右上 **「+」** → **「New repository」**
3. **Repository name** に `japowserch` など好きな名前を入力（ブランドに合わせるなら `japowserch` を推奨）
4. **Public** を選択
...
2. 開いた画面に、**JAPOWSERCH フォルダの中身**をドラッグ＆ドロップする  
...
2. **「Choose...」** を押して、**JAPOWSERCH フォルダ**（例: `デスクトップ\Cloude\JAPOWSERCH`）を選ぶ
...
→ JAPOWSERCH フォルダが GitHub Desktop に表示されます。
...
2. **Name** にリポジトリ名（例: `japowserch`）を入力
...
```
デスクトップ → Cloude → JAPOWSERCH
```

この **JAPOWSERCH** フォルダの中に、`ski-powder-hunter.html` や `README.md` などがある状態にします。
...
4. 次のように入力する：
   ...
   | 項目 | 入力例 | 説明 |
   |------|--------|------|
   | **Repository name** | `japowserch` または `ski-powder-hunter` | 好きな名前でOK（半角英数字とハイフン） |
...
1. **あなたの PC の JAPOWSERCH フォルダを「Git で管理する」**  
...
|------|----------------|----------------------------|
| ① | `cd ...`（下の 3-1） | 「これからは JAPOWSERCH フォルダで作業する」と指定する |
...
**打つ順番だけチェックリスト（JAPOWSERCH フォルダに移動したあと）**
...
### 3-1. ターミナルで JAPOWSERCH フォルダに移動する（＝①に当たる作業）
...
2. 次のコマンドを **1行ずつ** 入力して Enter：
   ...
   ※ フォルダを別のドライブに置いた場合は、エクスプローラーで JAPOWSERCH フォルダを開き、アドレス欄に表示されているパスを確認して、それに合わせて `cd` の後を書き換えてください。

   **別の方法（確実）：**
   - エクスプローラーで **JAPOWSERCH** フォルダを開く
   - アドレス欄をクリックしてパスを全選択（Ctrl+A）→ コピー（Ctrl+C）
   - PowerShell で `cd ` と入力したあと、**スペース1つ** 入れてから、貼り付け（Ctrl+V）
   - 例: `cd C:\Users\Takum\Desktop\Cloude\JAPOWSERCH`
   - Enter
...
3. **「HTTPS」** と書いてある下の **URL をコピー**する  
   例: `https://github.com/takum-skier/japowserch.git`
...
例（あなたの URL が `https://github.com/takum-skier/japowserch.git` なら）：

```powershell
git remote add origin https://github.com/takum-skier/japowserch.git
```
...
6. **「Save」** を押す
7. 少し待つ（1〜2分）と、上部に  
   **Your site is live at `https://○○○.github.io/japowserch/`** のような表示が出ます

### サイトの開き方

- **トップページ（ランキング）**:  
  `https://あなたのユーザー名.github.io/japowserch/ski-powder-hunter.html`  
  または  
  `https://あなたのユーザー名.github.io/japowserch/ski-powder-hunter.html` を **index.html にリネームして** 置くと、  
  `https://あなたのユーザー名.github.io/japowserch/` だけで開けます（任意）。
